...
class LevelScene extends Phaser.Scene {
  ...
  startFinishSequence(){if(this.finishStarted||this.collected<this.totalFlowers)return;this.finishStarted=true;this.finished=true;this.finishCameraX=this.cameras.main.scrollX;this.cameraTargetX=this.finishCameraX;this.cameras.main.stopFollow();this.cameras.main.scrollX=this.finishCameraX;this.finishCameraX=this.cameras.main.scrollX;this.cameras.main.stopFollow();this.cameras.main.scrollX=this.finishCameraX;this.player.body.setVelocity(0,0);/* v57: camera pan uitgeschakeld tijdens finish */this.time.delayedCall(420,()=>this.playFlowerGiftAnimation());}
  ...
  handleVariableJump(input,onGround){let s=window.FTTM.GameSettings,pressed=input.jump&&!this.jumpWasDown,released=!input.jump&&this.jumpWasDown;if(onGround){this.jumpCount=0;}if(released){this.jumpLocked=false;}if(pressed&&!this.jumpLocked){if(onGround){this.player.body.setVelocityY(s.jumpVelocity);this.jumpCount=1;this.jumpLocked=true;this.playJumpFeedback();}else if(this.jumpCount===1){this.player.body.setVelocityY(-560);this.jumpCount=2;this.jumpLocked=true;if(this.createDoubleJumpBurst)this.createDoubleJumpBurst();this.playJumpFeedback();}}if(released&&this.player.body.velocity.y<s.jumpCutVelocity)this.player.body.setVelocityY(s.jumpCutVelocity);this.jumpWasDown=input.jump;}
  createDoubleJumpBurst(){let x=this.player.x,y=this.player.y+38;for(let i=0;i<7;i++){let p=this.add.circle(x,y,3,0xffffff,.78).setDepth(40);this.tweens.add({targets:p,x:x+Phaser.Math.Between(-38,38),y:y+Phaser.Math.Between(8,42),alpha:0,scale:.2,duration:260,ease:'Sine.easeOut',onComplete:()=>p.destroy()});}}
...
// v57-double-jump-crash-fix marker

// build-marker: v57-double-jump-crash-fix

// build-marker: v57-double-jump-crash-fix

// build-marker: v57-double-jump-crash-fix-levelscene
...
// v57 double jump patch placeholder
