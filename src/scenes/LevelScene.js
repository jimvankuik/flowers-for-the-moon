...
class LevelScene extends Phaser.Scene {
  ...
  startFinishSequence(){if(this.finishStarted||this.collected<this.totalFlowers)return;this.finishStarted=true;this.finished=true;this.finishCameraX=this.cameras.main.scrollX;this.cameraTargetX=this.finishCameraX;this.cameras.main.stopFollow();this.cameras.main.scrollX=this.finishCameraX;this.finishCameraX=this.cameras.main.scrollX;this.cameras.main.stopFollow();this.cameras.main.scrollX=this.finishCameraX;this.player.body.setVelocity(0,0);/* v49: camera pan uitgeschakeld tijdens finish */this.time.delayedCall(420,()=>this.playFlowerGiftAnimation());}
  ...
  updateCamera(initial){
    ...
    if(this.isPortrait){
      ...
      else if(speed>35) targetAnchor=.20;
      else targetAnchor=.30;
    }else{
      if(speed<-35) targetAnchor=.54;
      else if(speed>35) targetAnchor=.32;
      else targetAnchor=.36;
    }
...
// v49-finish-camera-polish marker

// build-marker: v49-finish-camera-polish

// build-marker: v49-finish-camera-polish

// build-marker: v49-finish-camera-polish-levelscene
