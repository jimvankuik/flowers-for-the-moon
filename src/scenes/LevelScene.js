...
class LevelScene extends Phaser.Scene {
  ...
  startFinishSequence(){if(this.finishStarted||this.collected<this.totalFlowers)return;this.finishStarted=true;this.finished=true;this.finishCameraX=this.cameras.main.scrollX;this.cameraTargetX=this.finishCameraX;this.cameras.main.stopFollow();this.cameras.main.scrollX=this.finishCameraX;this.finishCameraX=this.cameras.main.scrollX;this.cameras.main.stopFollow();this.cameras.main.scrollX=this.finishCameraX;this.player.body.setVelocity(0,0);/* v55: camera pan uitgeschakeld tijdens finish */this.time.delayedCall(420,()=>this.playFlowerGiftAnimation());}
  ...
  handleVariableJump(input,onGround){let s=window.FTTM.GameSettings,pressed=input.jump&&!this.jumpWasDown,released=!input.jump&&this.jumpWasDown;if(onGround){this.jumpCount=0;this.jumpLocked=false;}if(pressed&&!this.jumpLocked){if(onGround){this.player.body.setVelocityY(s.jumpVelocity);this.jumpCount=1;this.jumpLocked=true;this.playJumpFeedback();}else if(this.jumpCount<2){this.player.body.setVelocityY(Math.min(s.jumpVelocity*.88,-500));this.jumpCount=2;this.createDoubleJumpBurst();this.playJumpFeedback();}}if(released&&this.player.body.velocity.y<s.jumpCutVelocity)this.player.body.setVelocityY(s.jumpCutVelocity);this.jumpWasDown=input.jump;}
...
// build-marker: v55-double-jump-active
...
// v55-double-jump-active marker

// build-marker: v55-double-jump-active

// build-marker: v55-double-jump-active

// build-marker: v55-double-jump-active-levelscene
...
// v55 double jump patch placeholder
